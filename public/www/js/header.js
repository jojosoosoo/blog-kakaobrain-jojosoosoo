...
window.addEventListener("load", function () {
  ...
  function showLine(_html, _tgY, _active, _scY) {
    // 브라우저의 스크롤 바의 위치를 파악해야 함
    // 현재 스크롤 바의 위치값 알아내기
    // scY = window.scrollY;
    // scY 즉, 스크롤 바의 위치가 0보다 크면 스크롤 된 거다
    if (_scY > _tgY) {
      // header 객체 즉, DOM에 css 목록에 추가 (클래스명)
      _html.classList.add(_active);
    } else {
      // header 객체 즉, DOM에 css 목록에 제거 (클래스명)
...
window.addEventListener("load", function () {
  ...
  // 2. 버튼 기능
  // 2.1 클릭하면 아이콘을 바꿈
  // 2.2 클릭하면 모바일 메뉴 및 배경을 보여줌
  mbBt.addEventListener("click", function (event) {
    // a 태그이므로 웹 브라우저가 갱신됨
    // a 태그가 작동이 안 되도록 기능을 막음
    ...
    if (mbMenuOpen) {
      ...
      // 메뉴 펼침 상태가 아닐 때 사용자가 클릭하면 메뉴를 펼침
